...
async def test_oauth_token_acquisition(monkeypatch):
    ...
    class FakeCtx:
        ...
        async def test_oauth_token_acquisition(monkeypatch):
            ...
            class FakeCtx:
                """Async context manager that yields the provided fake response."""
            ...
            class FakeSession:
                """Session that mimics aiohttp.ClientSession for post() only."""

                def __init__(self):
                    pass

                def post(self, url, data=None):
                    # NOTE: post must be a regular def returning an async-context manager
        ...
        async def test_oauth_refresh(monkeypatch):
            ...
            class FakeSession:
                ...
                def post(self, url, data=None):
                    # regular def that returns an async context manager
                    self._i += 1
                    if self._i == 1:
