...
try:
	with engine.begin() as conn:
		try:
			# Create alembic_version table if it does not exist with sufficiently large varchar
			r = conn.execute(text("SELECT to_regclass('public.alembic_version')"))
			exists = r.scalar()
			if not exists:
				print('start.sh: alembic_version table missing; creating with version_num VARCHAR(255)')
				...
				try:
					...
					print('start.sh: WARN: ALTER TABLE failed:', e)
		except Exception as e:
			print('start.sh: WARN: error ensuring alembic_version exists or is wide enough:', e)
		try:
			r2 = conn.execute(text("SELECT character_maximum_length FROM information_schema.columns WHERE table_name='alembic_version' AND column_name='version_num'"))
			val = r2.scalar()
			print('start.sh: alembic_version.version_num length after ensure =', val)
		except Exception as e:
			print('start.sh: WARN: could not inspect alembic_version column:', e)
finally:
	try:
		engine.dispose()
	except Exception:
